...
func (f *Formatter) legacyJSON(diff digest.Differences) error {
	...
	for _, addition := range diff.Additions {
		additions = append(additions, includes.String(addition, f.ctx.separator, f.ctx.rawSplit))
	}

	modifications := make([]string, 0, len(diff.Modifications))
	for _, modification := range diff.Modifications {
		modifications = append(modifications, includes.String(modification.Current, f.ctx.separator, f.ctx.rawSplit))
	}

	deletions := make([]string, 0, len(diff.Deletions))
	for _, deletion := range diff.Deletions {
		deletions = append(deletions, includes.String(deletion, f.ctx.separator, f.ctx.rawSplit))
	}
...
func (f *Formatter) json(diff digest.Differences) error {
	...
	for _, mods := range diff.Modifications {
		m := modification{Original: includes.String(mods.Original, f.ctx.separator, f.ctx.rawSplit), Current: includes.String(mods.Current, f.ctx.separator, f.ctx.rawSplit)}
...
func (f *Formatter) lineDiff(diff digest.Differences) error {
	...
	for _, addition := range diff.Additions {
		green(f.stdout, "+ %s\n", includes.String(addition, f.ctx.separator, f.ctx.rawSplit))
	}
	blue(f.stderr, "# Modifications (%d)\n", len(diff.Modifications))
	for _, modification := range diff.Modifications {
		red(f.stdout, "- %s\n", includes.String(modification.Original, f.ctx.separator, f.ctx.rawSplit))
		green(f.stdout, "+ %s\n", includes.String(modification.Current, f.ctx.separator, f.ctx.rawSplit))
	}
	blue(f.stderr, "# Deletions (%d)\n", len(diff.Deletions))
	for _, deletion := range diff.Deletions {
		red(f.stdout, "- %s\n", includes.String(deletion, f.ctx.separator, f.ctx.rawSplit))
	}
...
func (f *Formatter) wordLevelDiffs(diff digest.Differences, deletionFormat, additionFormat string, showUnchanged bool) error {
	...
	for _, addition := range diff.Additions {
		_, _ = fmt.Fprintln(f.stdout, green(additionFormat, includes.String(addition, f.ctx.separator, f.ctx.rawSplit)))
	}
	...
	for _, modification := range diff.Modifications {
		...
		_, _ = fmt.Fprintln(f.stdout, includes.String(result, f.ctx.separator, f.ctx.rawSplit))
	}

	_, _ = fmt.Fprintln(f.stderr, blue("# Deletions (%d)", len(diff.Deletions)))
	for _, deletion := range diff.Deletions {
		_, _ = fmt.Fprintln(f.stdout, red(deletionFormat, includes.String(deletion, f.ctx.separator, f.ctx.rawSplit)))
	}
	...
	if showUnchanged {
		...
		for _, unch := range diff.Unchanged {
			...
			_, _ = fmt.Fprintln(f.stdout, includes.String(unch, f.ctx.separator, f.ctx.rawSplit))
		}
	}
